...
We trained and thoroughly tested this framework on standard datasets like STL-10. The whole pipeline is built to be modular, so it's easy to run on remote servers with multiple GPUs. Based on our testing, combining these unique frequency features with standard deep learning techniques gives us highly competitive results while making the models much easier to interpret.
